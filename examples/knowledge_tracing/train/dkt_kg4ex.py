...
from config.dkt_kg4ex import config_dkt_kg4ex
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(parents=[setup_common_args()], description="DKT_KG4EX（预测学生学习下一个知识点的概率）的配置", add_help=False)
    # batch size
    ...
    parser.add_argument("--w_aux_loss", type=float, default=0.1)
    # 其它
    ...
    global_params, global_objects = config_dkt_kg4ex(params)
    ...
    global_objects["models"] = {
        "DKT_KG4EX": DKT_KG4EX(global_params, global_objects).to(global_params["device"])
    }
