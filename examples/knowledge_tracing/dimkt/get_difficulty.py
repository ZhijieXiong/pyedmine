...
def parse_difficulty(kt_data_, question2concept, func_params):
    num_min_question = func_params["num_min_question"]
    num_min_concept = func_params["num_min_concept"]
    num_question_diff = func_params["num_question_diff"]
    num_concept_diff = func_params["num_concept_diff"]
...
if __name__ == "__main__":
    ...
    # 对于出现次数小于num_min的习题和知识点，单独分配难度difficulty emb
    ...
    save_path = os.path.join(dimkt_dir, train_file_name.replace(".txt", "_dimkt_diff.json"))
    if not os.path.exists(save_path):
        ...
        difficulty_info = {"question_difficulty": q_acc, "concept_difficulty": c_acc}
