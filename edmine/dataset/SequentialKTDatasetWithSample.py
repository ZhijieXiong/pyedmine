...
class CLKTDataset(BasicSequentialKTDataset):
    ...
    def getitem_train_mode(self, index):
        ...
        correctness_seq_neg = CLKTSampler.negative_seq(user_data2aug["correctness_seq"], hard_neg_prob)
    ...
    def process_dataset(self):
        ...
        self.data_sampler = CLKTSampler(self.dataset_original)
    ...
    def get_random_aug(self, user_data2aug):
        ...
        for _ in range(num_aug):
            ...
            for aug_type in aug_order:
                if aug_type == "mask":
                    CLKTSampler.mask_seq(user_data_aug, mask_prob, 10)
                elif aug_type == "replace":
                    self.data_sampler.replace_seq(user_data_aug, replace_prob)
                elif aug_type == "permute":
                    CLKTSampler.permute_seq(user_data_aug, permute_prob, 10)
                elif aug_type == "crop":
                    CLKTSampler.crop_seq(user_data_aug, crop_prob, 10)
                else:
