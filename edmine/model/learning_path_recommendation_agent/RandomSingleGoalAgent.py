...
class RandomSingleGoalAgent(LPRAgent):
    ...
    def judge_done(self, memory, master_th):
        ...
        if len(memory.concept_rec_history) > max_stage:
            return True
        elif len(memory.concept_rec_history) == max_stage:
    ...
    def recommend_qc(self, memory, master_th):
        ...
        if len(memory.concept_rec_history) == 0:
            c_id2rec = self.rec_concept(memory)
        else:
    ...
    def a_star_knowledge_path(self, state, learning_goal):
        ...
        # 特殊情况：目标已掌握
        if state[learning_goal] >= threshold:
